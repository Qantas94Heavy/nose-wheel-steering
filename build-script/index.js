...
// jshint esversion:6, node:true, -W079
...
const optimized = new Promise(function (resolve, reject) {
  console.log(`Building Nose Wheel Steering: ${argv.debug ? 'debug' : 'release'} mode`);
...
fs.readFileAsync(path.join(config.baseUrl, 'userscript.js'), 'utf-8').then(function (file) {
  ...
  const extension = 'nws_v' + list.slice(0, 3).join('.') + (argv.debug ? '-debug' : '');

  minifying.then(function (minified) {
    minified += util.format(
      '\nvar a=window.nws={};a.version="%s";a.require=require;a.requirejs=requirejs;a.define=define',
      version
    );
